...
if len(sys.argv) == 6:
    ...
    print("Usage: python3 plot_hwloop_sat_consumption.py /path/to/src/ /path/to/dst/ job_per_sat sat_counter name_for_dir")
    exit()
...
base_xvalues = []
base_yvalues = []
with open(src + 'hwloop_total_consumption_satellite.csv', 'r') as infile:
...
plt.title('Total consumption per satellite')
...
plt.ylabel("Total consumption [mJ]")
plt.ylim(ymin, ymax)
...
plt.savefig(dst + 'hwloop_consumption_sat.svg', format='svg', bbox_inches="tight")
plt.savefig(dst + 'hwloop_consumption_sat.eps', format='eps', bbox_inches="tight", transparent=True)
plt.clf()
