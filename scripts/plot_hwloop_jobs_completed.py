...
if len(sys.argv) == 5:
    ...
    print("Usage: python3 plot_hwloop_jobs_completed.py /path/to/src/ /path/to/dst/ sat_counter name_for_dir")
    exit()
...
base_xvalues = []
base_yvalues = []
with open(src + 'hwloop_jobs_completed.csv', 'r') as infile:
...
ystep = 1 if max(base_yvalues) < 5 else max(base_yvalues) / 4
...
plt.title('Completed jobs per satellite')
...
plt.savefig(dst + 'hwloop_jobs_completed.svg', format='svg', bbox_inches="tight")
plt.savefig(dst + 'hwloop_jobs_completed.eps', format='eps', bbox_inches="tight", transparent=True)
plt.clf()
