...
int main(int c, char **v)
{
	...
	image in, out;
	if (c <= 5) {
		printf("Usage: %s ppm_in ppm_out size\n", v[0]);
        printf("ERROR");
		return -1;
	}
    ...
    strcat(datasetAddress, v[1]);
    strcat(datasetAddress, v[3]);
	in = read_ppm(datasetAddress);
	out = median_filter(in, size);
    strcat(resultAddress, v[2]);
    strcat(resultAddress, v[4]);
	write_ppm(out, resultAddress);
	free(in);
	free(out);
    printf("Completed!");
