...
def transpose_array(matrix):
    # Use zip to transpose the matrix and then convert the result to a list
...
if len(sys.argv) == 6:
    ...
    print("Usage: python3 plot_hwloop_time_per_task.py /path/to/src/ /path/to/dst/ sat_counter job_counter name_for_dir")
    exit()
...
if not os.path.exists(src + 'hwloop_time_per_task.csv'):
    sys.exit(-1)
...
with open(src + 'hwloop_time_per_task.csv', 'r') as infile:
...
for j in range(sat_counter):
    temp_xvalues = total_xvalues[j]
    temp_yvalues = total_yvalues[j]
    base_xvalues = transpose_array(temp_xvalues)
    base_yvalues = transpose_array(temp_yvalues)
    for m, elem in enumerate(base_xvalues):
        ...
        xstep = 1
        ymin = 0.0
        ymax = max(base_yvalues[m]) + min(base_yvalues[m]) / 3
        ...
        plt.title('Time working per task of satellite ' + str(j) + ' for job ' + str(m))
        plt.xlabel('Tasks')
        ...
        plt.grid(True)
        plt.savefig(dst + 'hwloop_time_per_task_of_satellite_' + str(j) + '_for_job_' + str(m) + '.svg', format='svg', bbox_inches="tight")
        plt.savefig(dst + 'hwloop_time_per_task_of_satellite_' + str(j) + '_for_job_' + str(m) + '.eps', format='eps', bbox_inches="tight", transparent=True)
        plt.clf()
        plt.close('all')
