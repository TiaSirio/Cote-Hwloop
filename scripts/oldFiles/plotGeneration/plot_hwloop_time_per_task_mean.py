...
if len(sys.argv) == 6:
    ...
    print("Usage: python3 plot_hwloop_time_per_task_mean.py /path/to/src/ /path/to/dst/ job_per_sat sat_counter name_for_dir")
    exit()
...
base_xvalues = []
base_yvalues = []
with open(src + 'hwloop_mean_time_per_task.csv', 'r') as infile:
...
plt.yticks([y for y in np.arange(ymin, ymax, ystep)],
           [str('{:.1f}'.format(y)) for y in np.arange(ymin, ymax, ystep)])
plt.grid(True)
# plt.savefig(dst + 'hwloop_consume_task.pdf', format='pdf')
plt.savefig(dst + 'hwloop_mean_time_task.svg', format='svg', bbox_inches="tight")
plt.savefig(dst + 'hwloop_mean_time_task.eps', format='eps', bbox_inches="tight", transparent=True)
plt.clf()
