...
if len(sys.argv) == 6:
    ...
    print("Usage: python3 plot_hwloop_time_per_job.py /path/to/src/ /path/to/dst/ sat_counter job_per_sat name_for_dir")
    exit()
...
if not os.path.exists(src + 'hwloop_time_per_job.csv'):
    sys.exit(-1)
...
total_xvalues = []
total_yvalues = []
with open(src + 'hwloop_time_per_job.csv', 'r') as infile:
...
for j in range(sat_counter):
    ...
    plt.title('Time working per job for satellite ' + str(j))
    plt.xlabel('Jobs')
    plt.xlim(xmin, xmax)
    ...
    plt.grid(True)
    plt.savefig(dst + 'hwloop_time_per_job_for_satellite_' + str(j) + '.svg', format='svg', bbox_inches="tight")
    plt.savefig(dst + 'hwloop_time_per_job_for_satellite_' + str(j) + '.eps', format='eps', bbox_inches="tight", transparent=True)
    plt.clf()
