...
modifyTimeToWait = maxTimeBeforeStopRadiatedProgram
                    ...
                    if temp in confMapRepeatProgram:
                        if confMapRepeatProgram[temp] is not None:
                            ...
                            for t in range(int(confMapRepeatProgram[temp])):
                                startInnerTimestamp = timestamp_ms()
                                try:
                                    ...
                                    if allInputs:
                                        timeToWaitTimeout = modifyTimeToWait * 3
                                    else:
                                        timeToWaitTimeout = modifyTimeToWait
                                    allInputs = False
                                    proc.wait(timeout=timeToWaitTimeout)
                                except Exception as e:
                                    # errorFound = True
                                    os.killpg(os.getpgid(proc.pid), signal.SIGTERM)
                                endInnerTimestamp = timestamp_ms()
                                innerDuration = startInnerTimestamp - endInnerTimestamp
                                modifyTimeToWait -= innerDuration
