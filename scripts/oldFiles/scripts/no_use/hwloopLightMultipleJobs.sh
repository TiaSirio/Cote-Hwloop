...
./mapperGeneratorMultipleJobs.sh
./configureCubeSatSimLightMultipleJobs.sh
result=$?
...
./hwloop.sh "$satellite" "$jobs" "m" "$mapperFile" "$physicalInstances" "$serverMQTT" "$portMQTT" "$subscriberID" "$publisherID" "$subscribeTopic" "$topicCommands" "$commandExecute" "$topicAppData" "$topicDuration" "$topicNonFuncData" "$topicPayload" "$noDataMessage"
