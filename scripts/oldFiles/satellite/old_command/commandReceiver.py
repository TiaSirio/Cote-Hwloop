...
if len(sys.argv) != 22:
    print("Missing argument: \"s for SingleJob or m for MultipleJobs\", \"Number of Physical satellite\", \"MQTT address\", \"MQTT port\", \"MQTT topic commands\", \"MQTT topic duration\","
          ...
          ", \"File non-functional\", \"File payload\", \"Tasks directory\", \"radiationErrorRatio\","
          "\"errorMessage\", \"conf file tasksExecutables\", \"conf file tasksDataset\" and \"conf file tasksResults\"!")
    sys.exit()
                ...
                for program in programsToExecute:
                    ...
                    if arguments[0].endswith(".py"):
                        ...
                        arguments.pop(0)
                            ...
                            # Insert random error by executing the program with a memory corruption program
                        #    execString = "./faint " + program
                        #else:
                        #    execString = "./" + program
                    ...
                    while True:
                        ...
                        if "Unique crashes:" in line:
                            ...
                            number_str = line[colon_index + 1:].strip()
                            errors = int(number_str)
                            if errors > 0:
                                errorFound = True
                        if not line:
                            break
