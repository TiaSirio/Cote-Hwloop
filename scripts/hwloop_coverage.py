...
if len(sys.argv) == 6:
    ...
    print(
        "Usage: python3 hwloop_coverage.py /full/path/to/src/ /full/path/to/dst/ 'Number of jobs' 'Number of satellites' 'dir_to_create'")
    exit()
...
with open(dst + 'hwloop_coverage.csv', 'w', newline='') as csvfile:
...
for j in range(sat_counter):
    ...
    for i in range(0, len(capture_matches)):
        counter += 1.0
    y_sat = float(counter) / float(job_per_satellite)
    with open(dst + 'hwloop_coverage.csv', 'a', newline='') as csvfile:
        csvwriter = csv.writer(csvfile, delimiter=',', quotechar='"')
