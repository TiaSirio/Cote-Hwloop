...
void sendMessageReceived(int instance, bool checkIfReceivedAll);
...
std::unordered_map<std::string, std::string> mapMessageArrived;
...
std::mutex msgReceivedMutex;
std::mutex saveMsgMutex;
std::mutex sendMsgSavedInMutex;
...
std::vector<std::vector<std::string>> queueThreads;
...

{
    ...
    void message_arrived(mqtt::const_message_ptr msg) override {
        std::lock_guard<std::mutex> lockReceivedMessage(msgReceivedMutex);
        // Save the incoming message to the static map
        mapMessageArrived[msg->to_topic()] = msg->to_string();
    }



        //const std::string topicReceived = msg->get_topic();
        //const std::string msgReceived = msg->to_string();
        // Retrieve id of satellite from the topic
        //std::vector<std::string> temp = split(topicReceived, "/");
        ...
        bool checkIfReceivedAll = false;
        if (true) {
            std::lock_guard<std::mutex> lockMessage(saveMsgMutex);
            queueThreads[temp[3]].push_back(msgReceived);

        }

        sendMessageReceived(temp[3], checkIfReceivedAll);
        ...
        if (taskSelected != nullptr){
            ...
            } else if ((topicReceived.rfind(topicNonFuncCopy, 0) == 0)) {
                // Handle non-functional data message
                if (msgReceived != ""){
                    std::lock_guard<std::mutex> lockTask(taskMutex);
                    taskSelected->setNonFuncData(msgReceived);
                    taskSelected->setTimestampsNonFuncData(msgReceived);
                    if (taskSelected->allValuesAddedOnTask()) {
                        addedAllTaskToJob = true;
                    }
                }
            ...
            } else if ((topicReceived.rfind(topicPayloadCopy, 0) == 0)) {
                // Handle payload data message
                if (msgReceived != ""){
                    std::lock_guard<std::mutex> lockTask(taskMutex);
                    taskSelected->setPayloadData(msgReceived);
                    taskSelected->setTimestampsPayloadData(msgReceived);
                    if (taskSelected->allValuesAddedOnTask()) {
                        addedAllTaskToJob = true;
                    }
                }
...
int main(int argc, char** argv) {
    ...
    /*std::vector<std::unique_ptr<mqtt::async_client>> cli_sub_array;
    for (int k = 0; k < numberOfSatellites; k++){
        const auto serverAddressCopy = SERVER_ADDRESS;
        int tempVal = static_cast<int>(k);
        const auto clientIdCopy = CLIENT_ID_SUB + std::to_string(static_cast<long>(tempVal));
        const std::string topicSubCopy = TOPIC_SUB + std::to_string(static_cast<long>(tempVal)) + "/#";

        if (true){
            std::lock_guard<std::mutex> lockConnectionSubscriber(connectionAtSubscriber);
            checkConnectionAtSubscriber[tempVal] = false;
        }

        // ASYNC SUBSCRIBE
        auto cli_sub = make_unique<mqtt::async_client>(serverAddressCopy, clientIdCopy);
        cli_sub_array.push_back(std::move(cli_sub));
        ...
        cli_sub.get()->set_callback(cb_sub);

        // Start the connection.
        // When completed, the callback will subscribe to topic.
        try {
            std::cout << "Sub client Connecting to the MQTT server...\n" << std::flush;
            cli_sub.get()->connect(connOpts_sub, nullptr, cb_sub);
        }
        catch (const mqtt::exception& exc) {
    ...
    // Install the callback(s) before connecting.
    callback cb_sub(cli_sub, connOpts_sub, TOPIC_SUB);
    ...
    */
    ...
    //std::vector<VectorType> threadQueues(numberOfSatellites, VectorType());
    /*for (int q = 0; q < numberOfSatellites; q++) {
        queueThreads.push_back(std::vector<std::string>{});
    }*/
    ...
    */
    ...
    /*std::cout << "\nDisconnecting subscribers... " << std::endl;
    for (int k = 0; k < numberOfSatellites; k++){
        cli_sub_array[k].get()->disconnect()->wait();
    }
    std::cout << "...Disconnected" << std::endl;*/
    ...
    while(simulate) {
        ...
        if(ehsPosn < PI) {
            ...
            if (cubePtr->isWorking()) {
                ...
                if (true) {
                    //std::unique_lock<std::mutex> lock(sendMsgSavedInMutex);
                    std::unique_lock<std::mutex> lock(waitForInstanceTurnMap);

                    cv3.wait(lock, [&idEhSat] {
...
void sendMessageReceived(int instance, bool checkIfReceivedAll) {
    std::lock_guard<std::mutex> lock(sendMsgSavedMutex);

    // Notify the waiting threads
    cv3.notify_all();
}

/*void manageMessageArrived(std::string topicReceived) {
    if (true){
        std::lock_guard<std::mutex> lockReceivedMessage(msgReceivedMutex);

        std::string topicReceived = msg->get_topic();
        size_t lastSlashPos = topicReceived.find_last_of('/');
        if (lastSlashPos != std::string::npos) {
            topicReceived.erase(lastSlashPos); // Erase the last '/'
        }
        std::string msgReceived = mapMessageArrived[topicReceived];
        // Retrieve id of satellite from the topic
        //std::vector<std::string> temp = split(topicReceived, "/");
    }
    // PhSat + Instance of Sat + Task
