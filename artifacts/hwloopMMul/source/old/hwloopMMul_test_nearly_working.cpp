...
void simulationThread(size_t idEhSat, size_t orbitDuration, double energyHarvestMaximumVolt, double energyHarvestMaximumCurrent, double energyStorageCapacity, double energyStorageEsr, satsim::Logger* logger, size_t numberOfJobs, std::vector<satsim::Job*> jobs, bool multipleJobs, int totalNumberOfSatellites);
...
std::unordered_map<int, bool> checkConnectionAtSubscriber;
...
std::mutex waitForPhysicalSatellite;
std::mutex waitForDataOfInstance;
std::mutex connectionAtSubscriber;
std::mutex accessListOfActiveInstances;

std::vector<int> availablePhysicalSatellite;
std::vector<int> concurrentWaitingPhysicalSatellites;
std::vector<int> instancesUsingPhysicalSatellite;

std::unordered_map<int, bool> instanceWaitingPhysicalSatelliteReceived;

std::condition_variable cv;
std::condition_variable cv2;

std::unordered_map<int, bool> instanceWaitingDataReceived;
...
class action_listener : public virtual mqtt::iaction_listener
{
    ...
    void on_success(const mqtt::token& tok) override {
        ...
        if (top && !top->empty()) {
            std::lock_guard<std::mutex> lockConnectionSubscriber(connectionAtSubscriber);
            checkConnectionAtSubscriber[satelliteUsed] = true;
        }
...
public:
    ...
    void addSatelliteUsedAtListener(int sat) {
        satelliteUsed = sat;
        name_ = name_ + std::to_string(sat);
    }
};
...

{
    ...
    int nretry_;
    int satelliteConnected;
    std::string topicToSubscribe;
    // The MQTT client
    ...
    void connected(const std::string& cause) override {
        ...
        //std::cout << "\nSubscribing to topic '" << topicToSubscribe << "'"
        ...
        cli_.subscribe(topicToSubscribe, QOS, nullptr, subListener_);
    }
    ...
    void message_arrived(mqtt::const_message_ptr msg) override {
        ...
        //std::cout << msg->get_topic() << std::endl;
        ...
        if (taskSelected != nullptr){
            ...
            if (checkValuesAdded) {
                ...
                for (int r = 0; r < tempVector.size(); r++){
                    ...
                    addedAllTasks = true;
                    //tasksToFinish[tempVector[r]]->printNonFuncData();
                }
            ...
            if (addedAllTasks){
                ...
                if (true) {
                    // Remove satellite that has finished to use physical satellites
                    std::lock_guard<std::mutex> lockUsedSatellites(accessListOfActiveInstances);
                    auto it = std::find(instancesUsingPhysicalSatellite.begin(), instancesUsingPhysicalSatellite.end(), jobSelected->getInstanceOfSatellite());
                    if (it != instancesUsingPhysicalSatellite.end()) {
                        instancesUsingPhysicalSatellite.erase(it);
                    }
                }
                ...
                if (true) {
                    ...
                    if (concurrentWaitingPhysicalSatellites.size() > 0) {
                        tempValue = concurrentWaitingPhysicalSatellites[0];
                        concurrentWaitingPhysicalSatellites.erase(concurrentWaitingPhysicalSatellites.begin());
                    }
                }
...
public:
    callback(mqtt::async_client& cli, mqtt::connect_options& connOpts, std::string topicToSubscribe, int satelliteConnected)
            : nretry_(0), cli_(cli), connOpts_(connOpts),
              topicToSubscribe(topicToSubscribe), satelliteConnected(satelliteConnected), subListener_("Sub")
    {
        subListener_.addSatelliteUsedAtListener(satelliteConnected);
    }
};
...
int main(int argc, char** argv) {
    ...
    */
    ...
    for (int i = 0; i < numberOfSatellites; ++i) {
        threads.push_back(std::thread(simulationThread, i, orbitDuration, energyHarvestMaximumVolt, energyHarvestMaximumCurrent, energyStorageCapacity, energyStorageEsr, &logger, numberOfJobs, jobsPerSat[i], multipleJobs, numberOfSatellites));
    }
    ...
    // Think if it useful to keep the satellite in the main thread in a vector, se they could be freed
    /*for(size_t i = 0; i < ehsatellites.size(); i++) {
        delete ehsatellites.at(i);
    }*/
    // Clean up jobs
    for(size_t i = 0; i < jobsPerSat.size(); i++) {
        for(size_t j = 0; j < jobsPerSat.size(); j++) {
...
bool physicalSatellitesAreBusy() {
    ...
    return availablePhysicalSatellite.empty();
}
...
void simulationThread(size_t idEhSat, size_t orbitDuration, double energyHarvestMaximumVolt, double energyHarvestMaximumCurrent, double energyStorageCapacity, double energyStorageEsr, satsim::Logger* logger, size_t numberOfJobs, std::vector<satsim::Job*> jobs, bool multipleJobs, int totalNumbersOfSatellites) {
    bool multipleJobsInner = multipleJobs;
    size_t publishPhSat = 1;

    const auto serverAddressCopy = SERVER_ADDRESS;
    const auto clientIdCopy = CLIENT_ID_SUB + std::to_string(idEhSat);
    int tempVal = static_cast<int>(idEhSat);
    const std::string topicSubCopy = TOPIC_SUB + std::to_string(static_cast<long>(tempVal)) + "/#";

    if (true){
        std::lock_guard<std::mutex> lockConnectionSubscriber(connectionAtSubscriber);
        checkConnectionAtSubscriber[idEhSat] = false;
    }

    // ASYNC SUBSCRIBE
    mqtt::async_client cli_sub(serverAddressCopy, clientIdCopy);

    mqtt::connect_options connOpts_sub;
    connOpts_sub.set_clean_session(false);

    // Install the callback(s) before connecting.
    callback cb_sub(cli_sub, connOpts_sub, topicSubCopy, tempVal);
    cli_sub.set_callback(cb_sub);
    ...
    try {
        ...
        std::cerr << "\nSub client ERROR: Unable to connect to MQTT server: '"
                  << serverAddressCopy << "'" << exc << std::endl;
        return;
    }

    std::string stringSubConnection = "Waiting for subscriber to subscribe at topic " + topicSubCopy + "...";
    std::cout << stringSubConnection << std::endl;
    //std::lock_guard<std::mutex> lockConnectionSubscriber(connectionAtSubscriber);
    while (!checkConnectionAtSubscriber[tempVal]);
    ...
    while(simulate) {
        ...
        if(ehsPosn < PI) {
                ...
                try {
                    if (true) {
                        ...
                        if (physicalSatellitesAreBusy()) {
                            ...
                            // Instances of satellites waiting at the moment
                            if (true) {
                                std::lock_guard<std::mutex> lockConcurrentAvailableSatellites(concurrentAvailableSatellitesMutex);
                                concurrentWaitingPhysicalSatellites.push_back(idEhSat);
                            }
                        } else {
                            // Take the number of physical satellite for which I could publish (here I remove the physical satellite working at the moment)
                            if (true) {
                                std::lock_guard<std::mutex> lockAvailableSatellites(availableSatellitesMutex);
                            ...
                            // Update list of satellites that uses physical satellites
                            if (true) {
                                std::lock_guard<std::mutex> lockUsedSatellites(accessListOfActiveInstances);
                                instancesUsingPhysicalSatellite.push_back(idEhSat);
                            }
                        }
                    }
                    ...
                    if (enterWaiting) {
                        ...
                        std::unique_lock<std::mutex> lock(waitForPhysicalSatellite);
                        if (!instanceWaitingPhysicalSatelliteReceived[idEhSat]) {
                            cv.wait(lock, [&idEhSat] {
                                return instanceWaitingPhysicalSatelliteReceived[idEhSat];
                            });
                        }
                        instanceWaitingPhysicalSatelliteReceived[idEhSat] = false;
                        lock.unlock();

                        // Remove the physical satellite from the array and use it
                        if (true) {
                            std::lock_guard<std::mutex> lockAvailableSatellites(availableSatellitesMutex);
                            publishPhSat = availablePhysicalSatellite[0];
                            availablePhysicalSatellite.erase(availablePhysicalSatellite.begin());
                        ...
                        // Update list of satellites that uses a physical satellite
                        if (true) {
                            std::lock_guard<std::mutex> lockUsedSatellites(accessListOfActiveInstances);
                            instancesUsingPhysicalSatellite.push_back(idEhSat);
                        }

                        // Remove working satellite from list of satellites in queue - DONE WHEN PASSING THE ARGUMENT TO cv
                    ...
                    // Save the physical satellite used and the remainingTasks to later be accessed when a message arrives
                    if (true) {
                        std::lock_guard<std::mutex> lockParameterMaps(parameterMapsMutex);
                        numberOfTasksPerJob[jobPtr] = remainingTasks;
                        physicalSatelliteUsed[jobPtr] = publishPhSat;
                        //physicalSatelliteUsed[jobPtr] = idEhSat;
                    }
                    ...
                    // To understand when a satellite has ended
                    if (true) {
                        std::lock_guard<std::mutex> lockJobFinished(jobFinishedMutex);
            ...
            /*auto ita = std::find(instancesUsingPhysicalSatellite.begin(), instancesUsingPhysicalSatellite.end(), idEhSat);

            if (ita == instancesUsingPhysicalSatellite.end()) {
            ...
            if (cubePtr->isWorking()) {
                ...
                if (waitingForValues) {
                    std::unique_lock<std::mutex> lock(waitForDataOfInstance);

                    if (!instanceWaitingDataReceived[idEhSat]) {
                        cv2.wait(lock, [&idEhSat] {
                            return instanceWaitingDataReceived[idEhSat];
                        });
                    }
                    instanceWaitingDataReceived[idEhSat] = false;
                    lock.unlock();
                ...
                if (true) {
                    std::lock_guard<std::mutex> lockPrint(printMutex);
                    if (print_value <= 0) {
                        print_value = 10000000;
                        std::string print_check = "POSITION IN THE ORBIT " + std::to_string(ehsPosn) + ", TRYING TO ACCESS THE JOB " + std::to_string(ehsPosn/radPerJob) + ". THE SATELLITE IS IDLE";
                        std::cout << print_check << std::endl;
                    }
                    print_value--;
                }
                // Otherwise proceed the simulation of 20 microseconds
                //std::cout << idEhSat << std::endl;
    ...
    if (true) {
        std::lock_guard<std::mutex> lock(waitForDataOfInstance);

        std::string satWaitingForTasksStr = "SATELLITES WAITING FOR TASKS: \n";
        for(int j = 0; j < totalNumbersOfSatellites; j++){
            std::string s = "- SAT: " + std::to_string(j) + " -> " + std::to_string(instanceWaitingDataReceived[j]) + "\n";
...
void sendInstanceFreedPhysicalSatellite(int instance) {
    std::lock_guard<std::mutex> lock(waitForPhysicalSatellite);
    instanceWaitingPhysicalSatelliteReceived[instance] = true;
...
void setUpPhysicalMap(int maxInstance) {
    ...
    for (int i = 0; i < maxInstance; i++){
        instanceWaitingPhysicalSatelliteReceived[i] = false;
    }
}

void sendInstanceCubeSatWorking(int instance) {
    std::lock_guard<std::mutex> lock(waitForDataOfInstance);
    instanceWaitingDataReceived[instance] = true;
...
void setUpCubeSatMap(int maxInstance) {
    std::lock_guard<std::mutex> lock(waitForDataOfInstance);
    for (int i = 0; i < maxInstance; i++){
        instanceWaitingDataReceived[i] = false;
    }
}