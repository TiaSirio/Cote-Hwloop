...
namespace satsim {
    ...
    void Logger::logEvent(const std::string& name, const double& time) {
        std::lock_guard<std::mutex> lockEvent(this->mutexLoggerEvent);
    ...
    void Logger::logConsumption(const std::string& name, const double& consumption) {
        std::lock_guard<std::mutex> lockConsumption(this->mutexLoggerConsumption);
        if(this->eventConsumption.count(name)==0) {
            this->eventConsumption[name] = std::vector<double>();
        }
        this->eventConsumption[name].push_back(consumption);
    }

    void Logger::logMeasurement(
            const std::string& name, const double& time, const double& measurement
    ) {
        std::lock_guard<std::mutex> lockMeasurement(this->mutexLoggerMeasurement);
    ...
    void Logger::exportCsvs(const std::string& pathToDir) {
        ...
        // for each consumption, write a csv file
        for(const auto& e : this->eventConsumption) {
            std::ofstream outFile;
            outFile.open(sanitizedPathToDir+"event-"+e.first+".csv");
            outFile << e.first << "Consumption (µJ),"
                    << std::endl;
            for(size_t i=0; i<e.second.size(); i++) {
                outFile << e.second.at(i) << ","
                        << std::endl;
