...
namespace satsim {
    ...
    void SimpleSolarCell::updateInstantaneousCurrent() {
        // When v(t) >= VMP, return 0 (VMP represents the voltage at which the solar cell operates with maximum power output)
        if(this->vmp_V <= this->getVoltage() && this->getCurrent() != 0.0) {
            ...
            // this->simTime_sec
            //);
        } else if(this->getVoltage() < this->vmp_V && this->getCurrent() != this->imp_A) {
